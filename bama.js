...
function nowConnectedToProducts(){
	...
	prompt.get([{name:'item_id',
		...
		// connection.query("UPDATE products SET ? WHERE ?", [{ stock_quantity: quantityOrdered }, {item_id: itemId }], 
		// 	function(err, res) {
		// 	
  ...
  //   //   		connection.query("select `item_id` `id`, `product_name` `Product`, `price` `Dollars`, `stock_quantity` from products", function(err, response){
		// 		// if(err){throw err};
		// 		// console.table("Updated Inventory",response);
